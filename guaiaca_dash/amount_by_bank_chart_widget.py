...
def generate_amoutbybank_chart_widget():
    ...
    try:
        ...
        grouped_sum = filtered_df.groupby('banco')['valor'].sum().reset_index()

        data = {
            'banco': grouped_sum['banco'].to_list(),
        ...
        fig = px.pie(df, values='valor', names='banco')
