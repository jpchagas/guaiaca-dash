...
def generate_amoutbyorigin_chart_widget():
    ...
    try:
        ...
        grouped_sum = filtered_df.groupby('origem')['valor'].sum().reset_index()

        data = {
            'origem': grouped_sum['origem'].to_list(),
        ...
        fig = px.pie(df, values='valor', names='origem')
